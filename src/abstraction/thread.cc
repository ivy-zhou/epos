...
// Class methods
void Thread::sleep(Thread::Queue &waiting) {
  assert(locked());
  while(_ready.empty()) {
    idle();
  }
  Thread *threadToBlock = _running;
  threadToBlock -> _state = WAITING;
  threadToBlock -> _waiting = &waiting;
  waiting.insert(&threadToBlock -> _link);

  // Reset running thread
  _running = _ready.remove() -> object();
  dispatch(threadToBlock, _running);
...
void Thread::wakeup(Thread::Queue &waiting) {
  assert(locked());

  if(!waiting.empty()) {
    Thread threadToUnblock = waiting.remove() -> object();
    threadToUnblock._state = READY;
    threadToUnblock -> waiting = 0;
    _ready.insert(&threadToUnblock.link);
  }
...
void Thread::wakeup_all(&queue<Thread> q) {
  while(!q-> empty()) {
    Thread *t = q -> remove() -> object();
    t -> state = READY;
    t -> _waiting = 0;
    _ready.insert(t -> link);
  }
